...
select employee_name, salary from employees where salary>50000 and hire_date < '2016-01-01';
...
select location_id, max(salary) as max_salary from Employees group by location_id;
...
select department_id, count(*) as number_of_employees from Employees group by department_id having count(*)<3 ;  

-- or --

select d.department_name as dept_name, count(*) as number_of_employees 
...
-- OR --
select l.location as location_name, avg(e.age) as average_age from Locations l
...
select l.location as location_name, e.employee_name from Employees e right join Locations l on l.location_id=e.location_id;
